"""Unit tests for fss_mcp.context — async logging and MCP notifications."""
...
class TestStderrLogging:
    ...
    async def test_log_info_writes_to_stderr_logger(self, caplog: Any) -> None:
        ...
        ctx = _make_context()
        with caplog.at_level(logging.DEBUG, logger="fss_mcp.handler"):
    ...
    async def test_log_warning_writes_to_stderr_logger(self, caplog: Any) -> None:
        ...
        ctx = _make_context()
        with caplog.at_level(logging.WARNING, logger="fss_mcp.handler"):
    ...
    async def test_log_error_writes_to_stderr_logger(self, caplog: Any) -> None:
        ...
        ctx = _make_context()
        with caplog.at_level(logging.ERROR, logger="fss_mcp.handler"):
    ...
    async def test_log_info_formats_args(self, caplog: Any) -> None:
        ...
        ctx = _make_context()
        with caplog.at_level(logging.INFO, logger="fss_mcp.handler"):
...
class TestFssAuthToken:
    ...
    def test_fss_auth_token_is_in_all_vars(self) -> None:
        from fss_core.fss_context import _ALL_VARS, fss_auth_token
