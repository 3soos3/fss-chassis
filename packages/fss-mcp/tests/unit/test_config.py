"""Unit tests for fss_mcp.config module."""
...
class TestDiscoveryPackagesConfig:
    """Tests for extensions.discovery_packages config key."""

    def test_default_discovery_packages_empty(self) -> None:
        config = ServerConfig()
        assert config.extensions.discovery_packages == ()

    def test_discovery_packages_parsed_from_toml(self, tmp_path: Path) -> None:
        toml_content = b"""
[extensions]
discovery_packages = ["pkg_a.tools", "pkg_b.tools"]
"""
        config_file = tmp_path / "config.toml"
        config_file.write_bytes(toml_content)
        config = ServerConfig.load(config_file)
        assert config.extensions.discovery_packages == ("pkg_a.tools", "pkg_b.tools")

    def test_discovery_packages_single_entry(self, tmp_path: Path) -> None:
        toml_content = b"""
[extensions]
discovery_packages = ["fss_mcp_solve_it.tools"]
"""
        config_file = tmp_path / "config.toml"
        config_file.write_bytes(toml_content)
        config = ServerConfig.load(config_file)
        assert config.extensions.discovery_packages == ("fss_mcp_solve_it.tools",)

    def test_discovery_packages_empty_list(self, tmp_path: Path) -> None:
        toml_content = b"""
[extensions]
discovery_packages = []
"""
