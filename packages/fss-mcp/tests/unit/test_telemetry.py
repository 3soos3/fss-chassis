"""Unit tests for fss_mcp.utils.telemetry — TelemetryManager setup."""
...
class TestTelemetryManagerDisabled:
    ...
    def test_disabled_by_default(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        from fss_mcp.utils.telemetry import TelemetryManager
    ...
    def test_start_span_returns_noop_when_disabled(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.delenv("MCP_OTEL_ENABLED", raising=False)
        from fss_mcp.utils.telemetry import TelemetryManager, _NoOpSpan
...
class TestTelemetryManagerEnabled:
    ...
    def test_resource_built_with_default_attrs(
        ...
        assert attrs["service.version"] == fss_mcp.__version__
    ...
    def test_init_logs_info_on_success(
        ...
        with caplog.at_level(logging.INFO, logger="fss_mcp.utils.telemetry"):
            TelemetryManager()
    ...
    def test_import_error_logs_warning(
        ...
        with caplog.at_level(logging.WARNING, logger="fss_mcp.utils.telemetry"):
            TelemetryManager()
    ...
    def test_init_exception_logs_error(
        ...
        with caplog.at_level(logging.ERROR, logger="fss_mcp.utils.telemetry"):
            TelemetryManager()
...
class TestNoOpSpan:
    ...
    def test_context_manager_protocol(self) -> None:
        from fss_mcp.utils.telemetry import _NoOpSpan
...
class TestGetTelemetrySingleton:
    ...
    def test_returns_same_instance(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("fss_mcp.utils.telemetry._manager", None)
        monkeypatch.delenv("MCP_OTEL_ENABLED", raising=False)
    ...
    def test_creates_telemetry_manager_instance(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setattr("fss_mcp.utils.telemetry._manager", None)
        monkeypatch.delenv("MCP_OTEL_ENABLED", raising=False)
        from fss_mcp.utils.telemetry import TelemetryManager, get_telemetry
