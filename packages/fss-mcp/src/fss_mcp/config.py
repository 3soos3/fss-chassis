...
class ExtensionSettings:
    ...
    Attributes:
        ...
        discovery_packages: Additional Python packages to scan for extensions.
            Each entry is a dotted package name (e.g. ``fss_mcp_solve_it.tools``).
            The package directory is scanned flat — .py files are imported directly,
...
def _build_security_config(raw: dict[str, Any]) -> SecurityConfig:
    ...
    """
    from fss_mcp.security.profiles import get_profile
...
def _build_extension_settings(raw: dict[str, Any]) -> ExtensionSettings:
    ...
    Returns:
        ExtensionSettings.
    """
    raw_pkgs = raw.get("discovery_packages", [])
    discovery_packages = tuple(str(p) for p in raw_pkgs)
