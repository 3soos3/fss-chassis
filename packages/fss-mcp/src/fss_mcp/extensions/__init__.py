...
def discover_extensions(
    ...
    Always scans the built-in tools/, resources/, and prompts/ subdirectories
    inside this extensions package. Additionally scans any packages listed in
    ``extra_packages`` (flat — no subdirectory traversal).

    Args:
        server: The ChassisServer to register extensions with.
        extra_packages: Dotted package names to scan for additional extensions.
    """
    ...
    for subdir in _SUBDIRS:
        ...
        if not subdir_path.is_dir():
            ...
            continue
        _scan_directory(subdir_path, f"fss_mcp.extensions.{subdir}", server)
...
def _scan_directory(directory: Path, module_prefix: str, server: ChassisServer) -> None:
    """Import all .py files in ``directory`` under the given module prefix.

    Args:
        directory: Filesystem path to scan.
        module_prefix: Dotted prefix prepended to each stem to form the module name.
        server: The ChassisServer to pass to each extension's register().
    """
