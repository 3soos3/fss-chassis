...
class ChassisServer:
    ...
    def __init__(self, config: ServerConfig) -> None:
        ...
        if config.diagnostics.health_check_enabled:
            from fss_mcp.diagnostics.health import register_health_check
    ...
    def _discover_extensions(self) -> None:
        ...
        try:
            from fss_mcp.extensions import discover_extensions

            discover_extensions(self, extra_packages=self._config.extensions.discovery_packages)
        except ImportError:
    ...
    def _make_error_result(self, exc: Exception, correlation_id: str = "") -> types.CallToolResult:
        ...
        if isinstance(exc, FSSCoreError):
    ...
    async def _dispatch_tool_inner(
        ...
        from fss_core.fss_context import fss_auth_token
        ...
        if not middleware_result.allowed:
            ...
            if "AUTH" in code:
                ...
                from fss_core.errors import FSS_REPLAY_REJECTED
        ...
        except FSSCoreError as exc:
            logger.error("Tool '%s' raised FSSCoreError: %s", tool_name, exc)
    ...
    async def run(self) -> None:
        ...
        if transport_name == "stdio":
            from fss_mcp.transport.stdio import StdioTransport

            self._transport = StdioTransport()
        elif transport_name == "http":
            from fss_mcp.transport.http import HTTPTransport
