"""Authentication and authorization framework for FSS-compliant servers.

Provides a pluggable AuthProvider ABC with built-in implementations:
- NoAuthProvider: always authenticates (for stdio/trusted environments)
- TokenAuthProvider: simple bearer token comparison (for HTTP use)
- ApiKeyProvider: SHA-256 hashed key store (FSS-0003 §5.1)
- OAuthJWTProvider: OAuth 2.0 JWT via JWKS endpoint (FSS-0003 §5.1 RECOMMENDED)
"""
...
class ApiKeyProvider(AuthProvider):
    ...
    async def authenticate(self, request_context: dict[str, Any]) -> AuthResult:
        ...
        if not raw_key:
            logger.warning("auth_failure: Missing Bearer token")
            return AuthResult.failure("Authentication required: no Bearer token")

        raw_key = raw_key.strip()
        if len(raw_key) < 32:
            logger.warning("auth_failure: Key too short")
        ...
        if identity_label is None:
            logger.warning("auth_failure: Key hash not found in store")
...
class OAuthJWTProvider(AuthProvider):
    ...
    async def authenticate(self, request_context: dict[str, Any]) -> AuthResult:
        ...
        if not token:
            logger.warning("auth_failure: Missing Bearer JWT")
            return AuthResult.failure("Authentication required: no Bearer token")
        ...
        try:
            ...
            logger.warning("auth_failure: JWT validation failed: %s", exc)
