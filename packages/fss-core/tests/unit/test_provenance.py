"""Unit tests for fss_core.utils.provenance — _provenance record (FSS-0004)."""
...
def _build(*args: Any, **kwargs: Any) -> dict[str, Any]:
    """Test helper: assert build_provenance_record returns a record (not None)."""
    result = build_provenance_record(*args, **kwargs)
    assert result is not None, "build_provenance_record returned None unexpectedly"
    return result
...
class TestBuildProvenanceRecord:
    ...
    def test_all_required_fields_present(self) -> None:
        record = _build("solveit_search", "1.0.0")
...
class TestFSSProvenanceDisabled:
    """Tests for FSS_PROVENANCE=false non-conformant mode."""
    ...
    def test_returns_none_when_provenance_disabled(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setenv("FSS_PROVENANCE", "false")
        result = build_provenance_record("tool", "1.0.0")
        assert result is None

    def test_returns_record_when_provenance_enabled(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        monkeypatch.setenv("FSS_PROVENANCE", "true")
        result = build_provenance_record("tool", "1.0.0")
        assert result is not None
        assert "transaction_id" in result

    def test_default_is_enabled(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.delenv("FSS_PROVENANCE", raising=False)
        result = build_provenance_record("tool", "1.0.0")
        assert result is not None
