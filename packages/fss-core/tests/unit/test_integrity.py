"""Unit tests for fss_core.utils.integrity — CAI, signing (FSS-0005)."""
...
class TestEnsureSigningKeyPair:
    ...
    def test_generates_key_pair_when_absent(self, tmp_path: pathlib.Path) -> None:
        ...
        from fss_core.integrity import ensure_signing_key_pair
    ...
    def test_kid_is_stable_for_same_key(self, tmp_path: pathlib.Path) -> None:
        pytest.importorskip("cryptography")
        from fss_core.integrity import _compute_key_id, ensure_signing_key_pair
...
class TestEd25519Signing:
    ...
    def test_sign_provenance_returns_dict_with_value_and_kid(self, signing_key: object) -> None:
        from fss_core.integrity import sign_provenance
    ...
    def test_load_and_use_generated_key(
        ...
        from fss_core.integrity import load_signing_key, sign_provenance
...
class TestValidateCAI:
    ...
    def test_valid_sha2_256(self) -> None:
        from fss_core.integrity import compute_json_cai, validate_cai

        cai = compute_json_cai({"x": 1})
        assert validate_cai(cai) is True

    def test_valid_sha2_384(self) -> None:
        from fss_core.integrity import compute_cai, validate_cai
    ...
    def test_unknown_algorithm_returns_false(self) -> None:
        from fss_core.integrity import validate_cai
